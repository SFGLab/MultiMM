...
class MultiEM:
    def __init__(self,chrom_ends,Cs=None,ms=None,ns=None,ks=None,ds=None,path='results/'):
        '''
    ...
    def add_forcefield(self):
        ...
        if np.all(self.Cs!=None) and len(np.unique(self.Cs)==2):
            ...
            self.comp_force.addGlobalParameter('Ea',defaultValue=-6.0)
            self.comp_force.addGlobalParameter('Eb',defaultValue=-3.0)
        ...
        self.container_force = mm.CustomExternalForce(
                '{}*max(0, r-{})^2; r=sqrt((x-{})^2+(y-{})^2+(z-{})^2)'.format(1000,5.0,self.mass_center[0],self.mass_center[1],self.mass_center[2]))
        for i in range(self.system.getNumParticles()):
    ...
    def run_pipeline(self,MD_steps=10000,run_MD=True,write_files=False,plots=False,build_init_struct=True,Temperature=310*mm.unit.kelvin,init_struct_path=None):
        ...
        integrator = mm.LangevinIntegrator(Temperature, 0.1, 1 * mm.unit.femtosecond)
        ...
        simulation.minimizeEnergy(tolerance=1e-2)
...
def main():
    ...
    # Load from files
    eigenvec, chr_ends = import_bw(bw_path="/mnt/raid/data/single_cell/coverage/k562.ATAC.merge.S.RPKM.bw",
                                   N_beads=N_beads,viz=False,binary=True,n_chroms=n_chrom,path=path)
    ms, ns, ds, ks, cs = import_mns_from_bedpe(bedpe_file='/mnt/raid/data/single_cell/PET_cluster_with_interchr/k562.S.all.bedpe',
