...
def import_bed(
    ...
    if chrom is None:
        for count, i in enumerate(chrom_idxs):
            comps_df[1][comps_df[0] == chrs[i]] = (
                comps_df[1][comps_df[0] == chrs[i]] + chrom_ends[count]
            )
            comps_df[2][comps_df[0] == chrs[i]] = (
                comps_df[2][comps_df[0] == chrs[i]] + chrom_ends[count]
            )
    ...
    resolution = (
        ...
        if chrom is None
        else (coords[1] - coords[0]) // N_beads
    )
    ...
    for i in tqdm(range(len(comps_df))):
        if comps_df[3][i].startswith("A.1") or comps_df[3][i].startswith("A1"):
            ...
            comps_df[3][i].startswith("A.2")
            or comps_df[3][i].startswith("A2")
            or comps_df[3][i].startswith("A")
        ):
            val = 1
        elif comps_df[3][i].startswith("B.2") or comps_df[3][i].startswith("B2"):
            val = -2
        elif (
            comps_df[3][i].startswith("B.1")
            or comps_df[3][i].startswith("B1")
            or comps_df[3][i].startswith("B")
        ):
            val = -1
...
def import_mns_from_bedpe(
    ...
    # Convert genomic coordinates to simulation beads
    resolution = (
        int(np.max(loops[5].values)) // N_beads
        if chrom is None
        else (coords[1] - coords[0]) // N_beads
    )
    chrom_ends = np.array(chrom_ends) // resolution
    ...
    if chrom is not None:
        loops[1], loops[2], loops[4], loops[5] = (
            loops[1] - coords[0],
            loops[2] - coords[0],
            loops[4] - coords[0],
            loops[5] - coords[0],
    ...
    mns, cs = (
        np.vstack(
            (
                loops["ms"].values[counts > threshold],
                loops["ns"].values[counts > threshold],
            )
        ),
    ...
    if cs.size == 0:
        raise ValueError(
            "The region of interest does not include loops. Please try with longer modelling region or increase the window around the gene."
        )
    ms, ns = mns[0, :], mns[1, :]
    ms[ms >= N_beads], ns[ns >= N_beads] = N_beads - 1, N_beads - 1
    ms, ns, cs = (
        ms[ns > ms + min_loop_dist],
        ns[ns > ms + min_loop_dist],
        cs[ns > ms + min_loop_dist],
    )
    ds = (
        0.1 + 0.1 * min_max_trans(1 / cs ** (2 / 3))
        if not np.all(cs == cs[0])
        else np.ones(len(ms))
    )
...
def import_bw(
    ...
    if chrom is None:
        ...
        genomewide_signal = np.nan_to_num(
            genomewide_signal, copy=True, nan=0.0, posinf=0.0, neginf=0.0
        )
    bw.close()
    ...
    if norm:
        genomewide_signal = (
            genomewide_signal
            - np.mean(genomewide_signal)
            + 3 * np.std(genomewide_signal)
        ) / np.std(genomewide_signal)
    ...
    if viz:
        ...
        plt.fill_between(
            xax,
            genomewide_signal,
            where=(genomewide_signal > np.mean(genomewide_signal)),
            alpha=0.50,
            color="purple",
        )
        plt.fill_between(
        ...
        plt.ylim(
            (
                np.mean(genomewide_signal) - np.std(genomewide_signal),
                np.mean(genomewide_signal) + np.std(genomewide_signal),
            )
        )
        plt.grid()
...
def get_gene_region(gene_tsv, gene_id=None, gene_name=None, window_size=200000):
    ...
    if gene_id is not None:
        ...
        if gene_name not in genes["gene_name"].values:
            raise ValueError(
                f"Gene name '{gene_name}' not found in the provided TSV file."
            )
...
def get_eigenvector(eigenvec_tsv, N_beads, chrom=None, region=None, viz=False):
    ...
    if chrom is not None:
        ...
        eigenvector = eigenvector[
            (eigenvector["end"] > region[0]) & (eigenvector["start"] < region[1])
        ].reset_index(drop=True)
        eigenvector["start"] = (eigenvector["start"] - region[0]) // resolution
        eigenvector["end"] = (eigenvector["end"] - region[0]) // resolution
    else:
        for i in range(n_chroms):
            eigenvector[eigenvector["chrom"] == chroms[i]]["start"] = (
                eigenvector[eigenvector["chrom"] == chroms[i]]["start"] + chrom_ends[i]
            )
            eigenvector[eigenvector["chrom"] == chroms[i]]["end"] = (
                eigenvector[eigenvector["chrom"] == chroms[i]]["end"] + chrom_ends[i]
            )
