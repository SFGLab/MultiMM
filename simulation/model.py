...
class MultiMM:
    def __init__(self, args):
        ...
        if args.GENE_ID is None and args.GENE_NAME is None and args.LOC_START is None:
            os.makedirs(
                os.path.join(self.save_path, "plots", "chromosomes"), exist_ok=True
            )
        ...
        if args.GENE_ID is None and args.GENE_NAME is None and args.LOC_START is None:
            os.makedirs(
                os.path.join(self.save_path, "model", "chromosomes"), exist_ok=True
            )

        chrom = args.CHROM
        coords = [args.LOC_START, args.LOC_END] if args.LOC_START is not None else None

        if (args.GENE_TSV is not None) and (
            str(args.MODELLING_LEVEL).lower() == "gene"
        ):
            if (
                args.GENE_ID is not None
                and str(args.GENE_ID).lower() != "none"
                and str(args.GENE_ID).lower() != ""
            ):
                ...
                self.gene_start, self.gene_end = (
                    (gene_coords[0] - coords[0]) * self.args.N_BEADS
                ) // (coords[1] - coords[0]), (
                    (gene_coords[1] - coords[0]) * self.args.N_BEADS
                ) // (
                    coords[1] - coords[0]
                )
                logger.info(
        ...
        if (
            args.COMPARTMENT_PATH is not None
            and str(args.COMPARTMENT_PATH).lower() != "none"
            and str(args.COMPARTMENT_PATH).lower() != ""
        ):
        ...
        if str(args.LOOPS_PATH).lower().endswith(".bedpe"):
            self.ms, self.ns, self.ds, self.chr_ends, self.chrom_idxs = (
                import_mns_from_bedpe(
            ...
            raise ValueError(
                "You did not provide appropriate loop file. Loop .bedpe file is obligatory."
            )

        # Nucleosomes
        if args.NUC_DO_INTERPOLATION and args.ATACSEQ_PATH is not None:
            if args.ATACSEQ_PATH.lower().endswith(
                ".bw"
            ) or args.ATACSEQ_PATH.lower().endswith(".bigwig"):
        ...
        # Chromosomes
        self.chrom_spin, self.chrom_strength = np.zeros(self.args.N_BEADS), np.zeros(
            self.args.N_BEADS
        )
        if (
            self.args.CHROM is None
            or self.args.CHROM == ""
            or str(self.args.CHROM).lower() == "none"
        ):
            for i in range(len(self.chr_ends) - 1):
                self.chrom_spin[self.chr_ends[i] : self.chr_ends[i + 1]] = (
                    self.chrom_idxs[i]
                )
                self.chrom_strength[self.chr_ends[i] : self.chr_ends[i + 1]] = (
                    chrom_strength[i]
                )

    def add_evforce(self):
        sigma = self.args.LE_HARMONIC_BOND_R0
        self.ev_force = mm.CustomNonbondedForce(
            f"epsilon*(sigma/(r+r_small))^{self.args.EV_POWER}"
        )
    ...
    def add_chromosomal_blocks(self):
        self.chrom_block_force = mm.CustomNonbondedForce(
            "E*(k_C*r^4-r^3+r^2); E=dE*delta(chrom1-chrom2)"
        )
    ...
    def add_loops(self):
        ...
        for m, n in tqdm(zip(self.ms, self.ns), total=len(self.ms)):
            if self.args.LE_FIXED_DISTANCES:
                self.loop_force.addBond(
                    m, n, self.args.LE_HARMONIC_BOND_R0, self.args.LE_HARMONIC_BOND_K
                )
            else:
                self.loop_force.addBond(
                    m, n, self.ds[counter], self.args.LE_HARMONIC_BOND_K
                )
            counter += 1
    ...
    def initialize_simulation(self):
        if self.args.BUILD_INITIAL_STRUCTURE:
            ...
            (
                "compartments"
                if np.all(self.Cs is not None) and len(np.unique(self.Cs)) <= 3
        ...
        self.pdb = (
            ...
            if self.args.INITIAL_STRUCTURE_PATH is None
            or self.args.BUILD_INITIAL_STRUCTURE
    ...
    def min_energy(self):
        ...
        try:
            ...
            logger.info(
                f"Failed to find {self.args.PLATFORM}: {e}. Falling back to CPU."
            )
        ...
        self.simulation = Simulation(
            self.pdb.topology, self.system, self.integrator, platform
        )
    ...
    def save_chromosomes(self):
        ...
        for i in range(len(self.chr_ends) - 1):
            write_mmcif_chrom(
                ...
                path=self.save_path
                + f"model/chromosomes/MultiMM_minimized_{chrs[self.chrom_idxs[i]]}.cif",
            )
    ...
    def nuc_interpolation(self):
        ...
        write_mmcif_chrom(
            Vnuc, path=self.save_path + "model/MultiMM_minimized_with_nucs.cif"
        )
        end = time.time()
        elapsed = end - start
        logger.info(
    ...
    def set_radiuses(self):
        self.radius1 = (
            (self.args.N_BEADS / 50000) ** (1 / 3)
            if self.args.SC_RADIUS1 is None
            else self.args.SC_RADIUS1
        )
        self.radius2 = (
            3.5 * (self.args.N_BEADS / 50000) ** (1 / 3)
            if self.args.SC_RADIUS2 is None
            else self.args.SC_RADIUS2
        )
    ...
    def make_plots(self):
        is_gw = (
            self.args.GENE_ID is None
            and self.args.GENE_NAME is None
        ...
        if is_gw:
            ...
            for i in range(len(self.chr_ends) - 1):
                ...
                viz_structure(
                    ...
                    cmap="coolwarm",
                    save_path=self.save_path
                    + f"plots/chromosomes/{chrs[self.chrom_idxs[i]]}_minimized_structure.png",
                )
        else:
            if self.args.GENE_ID is not None or self.args.GENE_NAME is not None:
                ...
                viz_gene_structure(
                    ...
                    cmap="coolwarm",
                    save_path=self.save_path
                    + "plots/initial_structure_gene_coloring.png",
                )
                ...
                viz_gene_structure(
                    ...
                    cmap="coolwarm",
                    save_path=self.save_path
                    + "plots/minimized_structure_gene_coloring.png",
                )
                if self.args.SIM_RUN_MD:
                    V = get_coordinates_cif(
                        self.save_path + "model/MultiMM_afterMD.cif"
                    )
                    viz_gene_structure(
                        V,
                        self.gene_start,
                        ...
                        cmap="coolwarm",
                        save_path=self.save_path
                        + "plots/structure_afterMD_gene_coloring.png",
                    )
            V = get_coordinates_cif(self.save_path + "metadata/MultiMM_init.cif")
            viz_structure(
                V,
            ...
            if is_comp:
                viz_structure(
                    ...
                    r=0.2,
                    save_path=self.save_path
                    + "plots/minimized_structure_compartment_coloring.png",
                )
            if self.args.SIM_RUN_MD:
                ...
                if is_comp:
                    viz_structure(
                        ...
                        r=0.2,
                        save_path=self.save_path
                        + "plots/structure_afterMD_compartment_coloring.png",
                    )
