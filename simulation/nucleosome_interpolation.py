...
def get_perp_component(x, v):
    """Component of x orthogonal to v.
...
class NucleosomeInterpolation:
    def __init__(self, V, bw, nuc_radius=0.1, points_per_nuc=20, phi_norm=np.pi / 5):
        self.V, self.bw = V, bw
        self.max_nucs_per_bead, self.nuc_points = (
            int(np.ceil(1 / (2 * nuc_radius))),
            points_per_nuc,
        )
    ...
    def move_structure_to(self, struct, p0, p1, p2):
        """The structure will be placed assuming new X-axis would be defined by
        vector p2-p1.

        New Y-axis would be defined by part of p0-p1 vector orthogonal
        to p2-p1.
        """
        ...
        if p1[0] == p0[0] and p1[1] == p0[1] and p1[2] == p0[2]:
            raise (
                Exception("Starting point and the reference point must be different!")
            )
    ...
    def interpolate_structure_with_nucleosomes(self, mode="random"):
        ...
        for i in tqdm(range(len(self.V) - 1)):
            ...
            if num_nucleosomes > 0:
                helices, prev_zigzag = self.single_bead_nucgenerator(
                    start_point, end_point, num_nucleosomes, prev_zigzag_vec=prev_zigzag
                )
        ...
        flattened_coords = [
            coord for sublist in interpolated_structure for coord in sublist
        ]
        logger.info("Done! You have the whole structure with nucleosomes. ;)")
        return np.array(flattened_coords)

    def single_bead_nucgenerator(
        self,
        start_point,
        end_point,
        num_nucleosomes,
        prev_zigzag_vec=None,
        turns=1.65,
        mode="random",
    ):
        ...
        for i in range(num_nucleosomes):
            ...
            zigzag_vec = (
                linker_len / 2 * (np.cos(phi) * zigzag_vec1 + np.sin(phi) * zigzag_vec2)
            )
            if mode == "random":
            ...
            if mode == "random":
                phi += np.pi + np.random.uniform(self.phi_norm, 2 * self.phi_norm) * (
...
def main():
    ...
    V = get_coordinates_cif(
        "/home/skorsak/Data/simulation_results/GM12878_GW/MultiMM_minimized.cif"
    )
    logger.info("Initial granularity of structure =", len(V))
    bw = import_bw(
        "/home/skorsak/Data/encode/ATAC-Seq/ENCSR637XSC_GM12878/ENCFF667MDI_pval.bigWig",
        len(V),
    )  # Mock self.signal array
