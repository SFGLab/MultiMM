    ...
    def convenient_argument_changer(self):
        ...
        if str(modelling_level).lower() in ("gene"):
            ...
            self.set_arg(
                ...
                self.args.COMPARTMENT_PATH != ""
                and str(self.args.COMPARTMENT_PATH).lower() != "none",
            )
            self.set_arg("IBL_USE_B_LAMINA_INTERACTION", False)
            self.set_arg("CF_USE_CENTRAL_FORCE", False)
            self.set_arg("SIM_RUN_MD", True)
...
def args_tests(args):
    if args.LOOPS_PATH is None or args.LOOPS_PATH == "":
        raise ValueError(
            "\033[91mMultiMM cannot run without providing interactions in .bedpe format!!!\033[0m"
        )
    elif (
        args.COMPARTMENT_PATH is None or args.COMPARTMENT_PATH == ""
    ) and args.COB_USE_COMPARTMENT_BLOCKS:
        raise ValueError(
        ...
        args.COMPARTMENT_PATH is None or args.COMPARTMENT_PATH == ""
    ) and args.SCB_USE_SUBCOMPARTMENT_BLOCKS:
        raise ValueError(
        ...
        raise ValueError(
            "\033[91mBetter disable CHB_USE_CHROMOSOMAL_BLOCKS when you model only one chromosome."
        )
    ...
    if (
        (not args.POL_USE_HARMONIC_BOND)
        or (not args.POL_USE_HARMONIC_ANGLE)
        or (not args.EV_USE_EXCLUDED_VOLUME)
    ):
        logger.warning(
...
def get_config():
    ...
    arg_parser.add_argument(
        "-c", "--config_file", help="Specify config file (ini format)", metavar="FILE"
    )
