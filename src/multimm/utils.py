    ...
    with open(filename, "w") as f:
        for name, value in args.model_dump().items():
            if isinstance(value, Quantity):
                f.write(f"{name} = {value._value} {value.unit.get_name()}\n")
            elif value is None:
                f.write(f"{name} = \n")
            else:
...
def get_eigenvector(eigenvec_tsv, N_beads, chrom=None, region=None, viz=False):
    ...
    if chrom is not None:
        ...
        eigenvector = eigenvector[(eigenvector["end"] > region[0]) & (eigenvector["start"] < region[1])].reset_index(
            drop=True
        )
        ...
        for i in range(n_chroms):
            mask = eigenvector["chrom"] == chroms[i]
            eigenvector.loc[mask, "start"] += chrom_ends[i]
            eigenvector.loc[mask, "end"] += chrom_ends[i]
